local hsluv = require("qkzk2.hsluv")
...
-- local ns = vim.api.nvim_create_namespace("qkzk2")
...
function util.debug(colors)
  colors = colors or require("qkzk2.colors")
  -- Dump unused colors
...
function util.onColorScheme()
  if vim.g.colors_name ~= "qkzk2" then
    vim.cmd([[autocmd! qkzk2]])
    vim.cmd([[augroup! qkzk2]])
  end
end
...
function util.autocmds(config)
  vim.cmd([[augroup qkzk2]])
  vim.cmd([[  autocmd!]])
  vim.cmd([[  autocmd ColorScheme * lua require("qkzk2.util").onColorScheme()]])
  if config.dev then
    vim.cmd([[  autocmd BufWritePost */lua/qkzk2/** nested colorscheme qkzk2]])
  end
